...
MOLE_BASE_SPEED = 2.7      # Forward speed - reduced for better control
...
TURN_DURATION = 0.49         # Fixed time to turn (seconds)
...
def compute_mole_velocity(mole_pos, wall_mask, current_time):
    ...
    if front_wall:
        ...
        if not right_wall:
            ...
            vl, vr = get_turn_velocities("CURVE_RIGHT")
...
def control_logic(sim):
    ...
    print("=" * 60)
    print("WALL FOLLOWING WITH PERSISTENT TURN STATE TRACKING")
    print("Strategy: Right-hand rule with timed turns")
    print("KEY: Turn state persists after turn completion")
    print("     Velocities return to normal after turn")
    print("=" * 60)
    print(f"\nTIMING PARAMETERS:")
    ...
    print("\nORIENTATION STATES (PERSISTENT):")
    print("0: Forward, 1: Turned Right, -1: Turned Left, 2: U-Turned")
    print("Note: State remains changed even after turn completes")
    print("\nStarting navigation...")
    ...
    try:
        while True:
            ...
            if frame_count % 3 == 0:
                ...
                cv2.imshow('Wall Following with Persistent Turn State', debug_image)
